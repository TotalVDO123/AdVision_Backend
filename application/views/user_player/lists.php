...
<section id="content">
      <section class="vbox">
        ...
        <section class="scrollable wrapper">
			...
			?>
				<div class="alert alert-warning <?= $add_class ?>">
				<strong>	Please create Play List and upload graphics/ video for new player</strong>
				</div>
		 ...
		 ?>
                ...
                <section class="hbox stretch">
                 <aside class="aside bg-white b-r" id="subNav">
				  <div class="wrapper b-b font-bold"> <strong><?php echo ucfirst( $user_full_name); ?></strong> All Players</div>
				  <?php /* ?>
				  <div class="wrapper b-b font-bold">All Players ( <?php echo $user_full_name; ?>)</div>
				  <?php */ ?>
				  <ul class="nav">
				  ...
				  ?>
                    ...
                    <table class="table table-striped m-b-none text-sm">
					  ...
					  <tbody>
						...
						///print_r($sections[$ii]['scrolling_text']);
						
						$scrolling_text=@$sections[$ii]['scrolling_text'];
						$rss_feeds=@$sections[$ii]['rss_feeds'];
						$show_scrolling=@$sections[$ii]['show_scrolling'];
						$show_rss=@$sections[$ii]['show_rss'];
						}
						?>
                        ...
                        <label>Scrolling Text</label>    
                            ...
                            <textarea class="form-control" name="scrolling_text" rows="1" data-minwords="3" data-required="true" placeholder="Scrolling text"> <?php echo $scrolling_text; ?></textarea>
						</div>
						...
						<span class="input-group-addon">
                           <input type="checkbox" value="1" name="showscrollingtext"  <?php if($show_scrolling==1){ ?> checked="checked" <?php } ?>>
                        </span>
						</div>
						</td>
						<tr>
                        ...
                        <label>&nbsp;</label>    
						<span class="input-group-addon">
                           <input type="checkbox" name="showrssfeeds" value="1" <?php if($show_rss==1){ ?> checked="checked" <?php } ?> >
                        </span>
						</div>
						</td>
						...
						<input type="submit" style="width:200px" class="btn btn-primary btn-block" value="Update" name="SUBMIT">
		...
		<div class="modal-content" >
                ...
                <div class="col-sm-12">
                  <section class="panel">
                    ...
                    <table class="table table-striped m-b-none text-sm" id="add_tr">
                      <thead>
                        <tr>
                          <th>Playlist</th>
                          <th>Day</th>                    
					   ...
					   <a href="javascript:void(0); " id="addplaylist" data-playlisturl="<?php echo base_url().'playlists/add/'?>" ><strong>ADD PLAYLIST</strong></a>
                        ...
                        <option value="SUN">Sunday</option>
                          ...
                          <td width="30%" >
                              ...
                              <select name="from_date>"  id="from_time" >
						  ...
						  <td class="text-right1">
						    ...
						    <?php
                              ...
                              $end=strtotime('23:00');
                              ?>
                              <select  name="to_date>" id="to_time">
                             <option value=""> Select</option>             
                               <?php
    ...
    $('.openPopup').on('click',function(){
		...
		var fields = aa.split('XXXX');
            ...
            $(".new_add").remove();
            	...
            	$.ajax({
			type: 'POST',
			url: '<?php echo base_url();?>section/get_playlist_schedule',
			data: "section_id="+$('#sectionid').val()+ "&playlist_id=" + $('select#playlistId').val(),
			
			success: function(response) 
			{
			   //$('#timing').html(response);
			   
			  // $('#add_tr > tbody:last-child').append(response);
			  
			  ///alert(response);
			  //$("#add_tr > tbody").append(response);
			  
			  //$('#add_tr > tbody:last').append('<tr>...</tr><tr>...</tr>'); 
...
<script>
	...
	if (confirm('Do you want to change the schedule of the play list?')) {
		...
		//}
			...
			data: "section_id="+$('#sectionid').val()+"&player_id="+$('#player_id').val()+"&from_date="+$('#from_time').val()+"&to_date=" + $('#to_time').val() + "&playlist_id=" + $('select#playlistId').val()+"&status="+$('#statusid').prop('checked')+"&day_name="+$('#day_name').val(),
			dataType: 'json',
			success: function(response) 
			{
