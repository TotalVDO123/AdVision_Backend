...
function load_view($url, $reqData=[]){
    $CI = get_instance();
	//if($CI->session->userdata('user_id')==1 or $CI->session->userdata('user_id')==2)
...
function load_digitalview($url, $reqData=[]){
    $CI = get_instance();
	//if($CI->session->userdata('user_id')==1 or $CI->session->userdata('user_id')==2)
	...
	function get_user_id($player_id=0)
	{
			$CI = get_instance();
			$user_id=0;
			...
			if(empty($player_id))
			{
				$player_id=$CI->session->userdata('player_id');
			}
			
			
			$query = "SELECT user_id from players where id='".$player_id."'";	
				
			$rec_set = $CI->db->query($query);
			if($rec_set->num_rows() > 0)
			{
				$player_rec  = $rec_set->result();
								
				$user_id= $player_rec[0]->user_id;
				return $user_id;		
			}
			return $user_id;		
	}
	...
	function get_username($user_id=0)
	{
			...
			if($rec_set->num_rows() > 0)
			{
				...
				$first_name= $player_rec[0]->first_name;
				$last_name= $player_rec[0]->last_name;
	...
	function load_view_userpart($url, $reqData=[]){
    $CI = get_instance();
	//if($CI->session->userdata('user_id')==1 or $CI->session->userdata('user_id')==2)
