   ...
   */
   ...
   public function player_login()
	...
	else
	{
        ...
        if ($res->num_rows() <= 0) {
            $JSON_ARR = array(
				'response'=>"There is not playlist in this section"
				);
		    print json_encode($JSON_ARR);	
			exit;
        }
			...
			foreach($sections as  $section )
			{
				...
				{
						$playlist_gallery=$this->playlists_model->GetPlaylistGallery($playlist['id']);
					...
					$playlist_content[]= array(
							...
							"name"=>$playlist['name'],
							"from_time"=>$playlist['from_time'],
							"to_time"=>$playlist['to_time'],
							"playlist_screen"=>$playlist_screen_content
						);
				
				
					$scrolling_text_content[]=array(
					"scrolling_text"=> $playlist['scrolling_text']
					);
