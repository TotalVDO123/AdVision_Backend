...
class Playlists extends Comman {
	...
	$data['row_playlist']='';
	$sql = "SELECT PL.name as playlist_name,
	PL.scrolling_text,
	PL.scrolling_check,
	PL.rss_feed,
	PL.rss_feed_check,
	U.company_name FROM  playlists PL left join players P on PL.playerId=P.id 
...
*/	
	...
	function update_scrolling_rssfeed()
	{
	

	  $scrolling_text=$this->input->post('scrolling_text');
      $scrolling_check=$this->input->post('scrolling_check');
      $rss_feed=$this->input->post('rss_feed');
      $rss_feed_check=$this->input->post('rss_feed_check');
      ...
      $playlist_id=$this->input->post('playlist_id');
         ...
         $data=array(
                ...
                'scrolling_check'=>$scrolling_check,
                'rss_feed'=>$rss_feed,
                'rss_feed_check'=>$rss_feed_check
                );
            
            $this->db->update('playlists', $data,  array('id'=>$playlist_id));
	...
	function playlistschedule($schedule_id=0)
	{
		...
		//$sql = "SELECT * FROM  playlists WHERE 1 order by name ";
			$sql = "SELECT * FROM  playlists WHERE 1 and user_id='".$user_id."' order by name ";
	...
	{	
			...
			//$sql ="SELECT count(S.id) as screen_no,U.id as user_id,U.company_name FROM screens S left join users U on S.user_id= U.id group by U.id having count(S.id)>0";
			
			$sql ="SELECT count(S.id) as screen_no,U.id as user_id,U.company_name FROM screens S left join users U on S.user_id= U.id where S.user_id='".$this->session->userdata('tmp_user_id')."' group by U.id having count(S.id)>0";
			...
			else
			{
					...
					$sql ="SELECT count(S.id) as screen_no,U.id as user_id,U.company_name FROM screens S left join users U on S.user_id= U.id 
	...
	function schedules()
	{
	   ...
	   $user_id=$this->session->userdata('user_id');
		...
		$sql = "SELECT * FROM  schedule WHERE 1 and user_id = '".$user_id."'";
		if($this->session->userdata('user_type')==1 )
		{
		$sql = "SELECT * from schedule WHERE 1 and  user_id = '".$this->session->userdata('tmp_user_id')."' order by id";
		}
	...
	function addplaylists()
	{
		...
		load_digitalview('Playlists/add_playlists', $result);
	}
	
	
	function editplaylist($playlist_id=0)
	{
		
			$sql = "SELECT * FROM   playlists WHERE 1 and  id='".$playlist_id."'";
			$resdata = $this->db->query($sql);
			$result['playlist']="";
			if ($resdata->num_rows() > 0) 
			{
				$result['playlist'] = $resdata->result_array();
			}
		...
		if($reqData)
		{
			...
			$this->db->update('playlists', $data,  array('id' =>  $reqData['playlist_id']));
			 //$this->db->insert('schedule', $data);
			redirect('Playlists/editplaylist/'.$reqData['playlist_id']);
		}	
		
		
		load_digitalview('Playlists/edit_playlists', $result);
	}
