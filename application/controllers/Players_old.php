...
class Players extends Comman {
	...
	public function index(){
		...
		$result['result1']=$this->players_model->getAll($user_id);
	  ...
	  /// echo "===========".$this->db->last_query();
	   ...
	   $result['assign_users']=$rest->result_array();
		
		$sql="select  players.*,'1' as 'assign' from player_assign_user left join players on player_assign_user.player_id=players.id  where player_assign_user.user_id='".$user_id."'" ;
		$query = $this->db->query($sql);
		$assihn_player= $query->result_array();
		
		$result['assign_player']=$assihn_player;
		
	$result['result']=	array_merge($assihn_player,$result['result1']);
	...
	public function player(){
	   ...
	   load_view('user_player/lists', $result);
		...
		public function playerassign_to_user()
		{
	        ...
	        foreach($user_id as $row)
	        {
	           $data=array(
	            ...
	            $inserted= $this->db->insert('player_assign_user',$data);
	        }
